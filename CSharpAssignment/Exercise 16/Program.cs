    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                Console.Write("Enter the directory (example: F:) ");
                ...
                if (Directory.Exists(directoryPath))
                {
                    ...
                    var extensionTypeCounters = directoryInfo.EnumerateFiles(".", SearchOption.TopDirectoryOnly).GroupBy(file => file.Extension).Select(g => new { Extension = g.Key, Count = g.Count() }).ToList();
