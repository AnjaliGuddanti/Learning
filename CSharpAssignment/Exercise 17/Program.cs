...
namespace Exercise_17
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (nTimesPlayed < 5)
                ...
                { 
                    Console.WriteLine(ex.Message); userNumber = -1; 
                }
        }
    ...
    static void getUserAnswer(string msg, int selectedMethod)
        ...
        static bool isCorrectAnswer(string msg, int minNumber, int maxNumber, int selectedMethod, bool hasNextFunctions)
        {
            ...
            {
                int number = -1; int.TryParse(msg, out number);
                if (selectedMethod == 1)
                { 
                    if (number % 2 == 0)
                    { 
                ...
                if (selectedMethod == 5) { if (number == 0) { return true; } else { throw new InvalidNumberException("\nIt is not a zero number.\n"); } }
                return false;
            }
        }
